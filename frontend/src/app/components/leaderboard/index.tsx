...
const Leaderboard = () => {
  ...
  const investors = useMemo<Investor[]>(() => [
    ...
    { rank: 11, username: "karen93.lens", avatar: "👤", signals: 60, value: 2263.91, performance: 3 },
    { rank: 12, username: "unitswamp90.lens", avatar: "👤", signals: 85, value: 7532.78, performance: 11 },
    { rank: 13, username: "stephen.xp.lens", avatar: "👤", signals: 85, value: 4319.31, performance: 9 },
    { rank: 14, username: "0+3a5b...651e", avatar: "👤", signals: 82, value: 3392.18, performance: 8 },
    { rank: 15, username: "arthur457.lens", avatar: "👤", signals: 84, value: 3873.19, performance: 7 },
    { rank: 16, username: "rfthomas.lens", avatar: "👤", signals: 75, value: 3752.73, performance: 7 },
    { rank: 17, username: "imfrancis.lens", avatar: "👤", signals: 79, value: 3300.45, performance: 7 },
    { rank: 18, username: "0+1d3c...158a", avatar: "👤", signals: 69, value: 3291.78, performance: 7 },
    { rank: 19, username: "0+4b6c...862f", avatar: "👤", signals: 67, value: 2900.38, performance: 6 },
    { rank: 20, username: "fester76.lens", avatar: "👤", signals: 59, value: 2401.10, performance: 3 },
    { rank: 21, username: "karen93.lens", avatar: "👤", signals: 60, value: 2263.91, performance: 3 },
    { rank: 22, username: "karen93.lens", avatar: "👤", signals: 60, value: 2263.91, performance: 3 },
  ], []);
      ...
      <div className="max-w-8xl mx-auto p-8 pt-4">
        <div className="bg-white rounded-lg shadow-sm p-6 mt-4">
                ...
                <tbody>
                  {currentInvestors.map((investor) => (
                    <tr key={investor.rank} className="border-b border-gray-100">
                      <td className="py-4 h-16 text-gray-700">{investor.rank}</td>
                      <td className="py-4 h-16">
                        <div className="flex items-center">
                          <div className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center mr-2">
                            {investor.avatar}
                          </div>
                          <span className="text-gray-700 font-bold">{investor.username}</span>
                        </div>
                      </td>
                      <td className="py-4 h-16 text-center text-gray-700">{investor.signals}</td>
                      <td className="py-4 h-16 text-gray-700">$ {investor.value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                      <td className="py-4 h-16">
          ...
          <div className="mt-4 flex flex-col sm:flex-row items-center justify-between">
            <span className="text-sm text-gray-500 mb-4 sm:mb-0 font-bold">
              <span className="font-normal">Showing</span> {filteredInvestors.length > 0 ? indexOfFirstInvestor + 1 : 0}-{Math.min(indexOfLastInvestor, filteredInvestors.length)} <span className="font-normal">of</span> {filteredInvestors.length}
            </span>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mt-6">
                ...
                <tbody>
                  {currentActivities.map((activity, index) => (
                    <tr key={index} className="border-b border-gray-100">
                      ...
                      <td className="py-4 h-16 text-right text-gray-500">{activity.date}</td>
                    </tr>
                  ))}
                  {/* Empty rows to maintain fixed height when fewer items */}
                  {emptyActivityRows.map((_, index) => (
                    <tr key={`empty-activity-${index}`} className="border-b border-gray-100">
                      <td className="h-16"></td>
                      <td className="h-16"></td>
                    </tr>
                  ))}
          ...
          <div className="mt-2 flex flex-col sm:flex-row items-center justify-between">
            <span className="text-sm text-gray-500 mb-4 sm:mb-0 font-bold">
              <span className="font-normal">Showing</span> {indexOfFirstActivity + 1}-{Math.min(indexOfLastActivity, activities.length)} <span className="font-normal">of</span> {activities.length}
            </span>
            <div className="flex-grow flex justify-center">
              <Pagination 
