-- +goose Up
CREATE SCHEMA IF NOT EXISTS gorillioner;

-- Create the events table
CREATE SCHEMA IF NOT EXISTS gorillioner;

CREATE TABLE IF NOT EXISTS gorillioner.currency (
...
CREATE TABLE IF NOT EXISTS gorillioner.pairs (
    id SERIAL PRIMARY KEY,
    ...
    CONSTRAINT fk_base_currency
        ...
        REFERENCES gorillioner.currency(id)
        ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS gorillioner.events (
...
CREATE INDEX IF NOT EXISTS idx_events_timestamp ON gorillioner.events (timestamp DESC);

-- Create a GIN index on the JSONB data column for faster JSON queries
CREATE INDEX IF NOT EXISTS idx_events_data ON gorillioner.events USING GIN (data); 

-- +goose Down
DROP INDEX IF EXISTS idx_events_data;
DROP INDEX IF EXISTS idx_events_timestamp;

DROP TABLE IF EXISTS gorillioner.events CASCADE;
DROP TABLE IF EXISTS gorillioner.pairs CASCADE;
DROP TABLE IF EXISTS gorillioner.currency CASCADE;
DROP SCHEMA IF EXISTS gorillioner CASCADE;