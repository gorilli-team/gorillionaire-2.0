...
function getTrackingTimeString(timestamp: number) {
...
router.get(
  ...
  async (
    req: Request<
      ...
      {},
      {},
      { page: string; limit: string; type: string; impact: string }
    >,
    res: Response
  ) => {
    try {
        ...
        const transferEvents: TokenEvent[] = transfers.map((transfer) => {
          ...
          return {
            ...
            timestamp: transfer.blockTimestamp
              ...
              : new Date(transfer.timestamp || 0).toLocaleString(),
        ...
        const spikeEvents: TokenEvent[] = allSpikes.map((spike) => {
          ...
          return {
            ...
            timestamp: spike.blockTimestamp
              ? new Date(spike.blockTimestamp * 1000).toLocaleString()
              : new Date(spike.timestamp || 0).toLocaleString(),
            description: `${
              spike.tokenSymbol
...
(+${increasePercentage.toFixed(2)}%)`,
      ...
      const oldestEvent = filteredEvents.reduce((oldest, current) => {
        return parseInt(current.blockTimestamp) <
          parseInt(oldest.blockTimestamp)
          ? current
          : oldest;
      }, filteredEvents[0]);
